...
public class MummyRed : MonoBehaviour
{
    ...
    public void DiChuyenMummy()
    {
        ...
        if (getPlayerturn == false) //Nếu player đã di chuyển => đến lượt mummy di chuyển
        {
            ...
            if (Mathf.Approximately(posPlayer.y, posMummy.y))
            {
                posMummy.y = posPlayer.y;
            }

            //Start MOVE
            if (posPlayer.y > posMummy.y)
            {
    ...
    void Update()
    {
        if (flagMove)
        {
            if (Mathf.Approximately(this.transform.position.x, nextPoint.x) && Mathf.Approximately(this.transform.position.y, nextPoint.y))
            {
                ...
                //Set lai animation 
                int currentstate = anim.GetInteger("mummyredstate");
                anim.SetInteger("mummyredstate", currentstate + 3);
    ...
    public void MoveUp()
    {
        if (flagMove == false)
        {
            sobuocdi++;
            flagMove = true;
            anim.SetInteger("mummyredstate", 1);
            if (isCanMove(sightUp) == false)
    ...
    public void MoveDown()
    {
        if (flagMove == false)
        {
            sobuocdi++;
            flagMove = true;
            anim.SetInteger("mummyredstate", 0);
    ...
    public void MoveLeft()
    {
        if (flagMove == false)
        {
            sobuocdi++;
            flagMove = true;
            anim.SetInteger("mummyredstate", 2);
            //Check can move
            if (isCanMove(sightLeft) == false) nextPoint = new Vector2(transform.position.x, transform.position.y);
            else
            {
                AudioSource.PlayClipAtPoint(walk, transform.position, 1.0f);
